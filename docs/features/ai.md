...
直播总结支持在生成完成后追加写入飞书云文档。开启位置：设置 -> AI配置 -> 功能 -> 直播总结 -> 写入飞书。

配置项：

1. `App ID`：飞书开放平台企业自建应用的 App ID。
2. `App Secret`：同一个应用的 App Secret。
3. `文档 ID/链接`：可以填写完整的飞书 `docx` 文档链接，也可以直接填写链接中 `/docx/` 后面的 Document ID。
