...
2. B站视频审核状态检测通知
3. 支持DDTV webhook，**需要将DDTV的软件“设置-文件与路径设置-录制文件保存路径”设置为绝对路径**
4. 弹幕转换新增“时间偏移”参数
5. 斗鱼下载支持清晰度选择、不下载弹幕、覆盖控制选项
6. B站下载支持覆盖控制选项
7. ffmpeg参数支持视频滤镜选项
8. 支持硬编码时间参数到视频中，优先从webhook中读取、其次是视频元数据（如录播姬注释），即使你开启此选项，也是有可能不被渲染的
9. amf相关编码支持硬件解码
10. 在上传弹幕版本时同时支持上传非弹幕版
