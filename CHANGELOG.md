...
3. 新增选项支持自动更新账号授权
4. 用户页新增复制cookie按钮
5. B站录制支持hls以及fmp4的avc以及hevc，**hls及fmp4流录制一个小时后被自动分段，如果你确定主播的流没有flv，请确保分段时间不为一个小时**
6. 上传支持审核通过后删除选项
7. B站录制支持“画质匹配重试次数”选项
...
3. 优化配置文件导入
4. 优化配置结构
5. 斗鱼弹幕录制增加自动重连以及错误报告
6. 弹幕预设保存增加验证
7. 录制命名规则修改增加提示避免错误修改
8. 优化webhook中上传的限制时间的实现，修改为加入队列，之后自动启动
9. 切片导出时支持忽略弹幕选项
