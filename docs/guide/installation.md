...
| `TZ`                     | 时区设置                           | 否   | -      |
| `PUID`                   | 运行进程的用户ID，用于文件权限管理 | 否   | 0      |
| `PGID`                   | 运行进程的组ID，用于文件权限管理   | 否   | 0      |
| `UMASK`                  | 文件创建权限掩码                   | 否   | 022    |
...
::: warning 权限管理
设置 `PUID` 和 `PGID` 可以确保容器创建的文件具有正确的所有者权限。建议设置为宿主机用户的 UID 和 GID（可通过 `id` 命令查看）。例如：
...
::: tip UMASK 说明
`UMASK` 用于控制新创建文件的权限。常用值：

- `0022`: 文件权限为 644 (rw-r--r--)，目录权限为 755 (rwxr-xr-x)
- `0002`: 文件权限为 664 (rw-rw-r--)，目录权限为 775 (rwxrwxr-x)
  :::
