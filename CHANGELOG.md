...
从 1.0 走来，本软件绝大多数的目标都已经实现，从重构支持docker，到支持切片，再到本来没想实现的直播录制，最后到这个版本的文件同步，只剩下最后一个弹幕分析还没完成，可以说直播录制大部分需求都可以在本软件中操作，也有一些是 only GUI can do，祝所有录播man可以更加方便来处理视频。

## 破坏性更新

- **注意：移除“不压制后处理”参数，如果之前存在视频配置可能会导致错误调用，配置删除选项可能会误处理你的文件**
...
- 移除“不压制后处理”参数，如果之前存在视频配置可能会导致错误调用
