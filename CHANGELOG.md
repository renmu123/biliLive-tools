...
1. 弹幕转换&转封装增加“让我选择”文件夹功能
2. 为弹幕转换增加屏蔽词功能，如果出现开启后无法转换的情况请反馈
3. 增加版本更新记录查看
4. 在webhook弹幕转换时复制输入文件到临时文件夹来避免某些情况下danmafactory读取文件失败，比如文件名有emoji
5. 弹幕转换任务支持取消操作
...
1. 修改弹幕为空的判断逻辑，如果文件中存在</d>, </gift>, </sc>, </guard>任意标签则认为不为空
2. 修复“弹幕转换”页面的文件选择栏错误
3. 修复blrec的webhook无法使用的bug
4. 修复深色模式下的续传功能UI
