...
1. 硬件解码又被加回来了，仅在未使用滤镜被添加
2. 虎牙录制支持精细画质
3. 支持抖音直播录制
4. 转码功能支持高能进度条
5. 支持导入导出弹幕配置
6. 弹幕文字支持根据分辨率进行自适应
7. 移除webhook中的“使用视频文件名”参数，模板中增加`{{filename}}`参数，移除了标题的不再维护警告
...
1. 虎牙录制配置时不显示“高能弹幕”配置
2. 设置选项的tab可以被记住
3. 录制支持直接打开webhhook配置页
4. 切片页支持关闭视频
5. 单独弹幕转换开启“自适应分辨率”选项时，增加额外提示
6. B站的合集和分区增加缓存
7. 优化减少部分页面的多余接口请求
8. 优化B站hls流的录制，ts流比fmp4优先级更高
9. B站录制弹幕服务增加自动重试
10. ffmpeg预设增加分辨率验证
11. B站录制"避免hls自动分段"选项默认开启
12. 斗鱼礼物弹幕支持“开通钻粉”和“续费钻粉”
13. 虎牙画质支持蓝光20M,蓝光10M
14. "调试模式"参数默认为关闭
15. 录制列表模式部分字段添加排序
16. 录制：`画质匹配重试次数` 支持`-1`参数，用于强制匹配画质，虎牙支持该参数

## Bug修复

1. 修复添加虎牙录播时，默认画质参数错误的bug
2. 修复虎牙录制画质未生效的bug
3. 修复斗鱼录制全局画质未获取到的bug
4. 虎牙录制“星秀”分区直播不再破碎，也有可能带来其他问题，如果有请反馈
5. 修复二次编辑时话题未被保存的bug [#51](https://github.com/renmu123/biliLive-tools/issues/51)
6. 修复启动后进入时视频预设可能未正常显示的bug
7. 修复B站录制代理未生效的bug
