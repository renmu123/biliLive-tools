...
用于在webhook中收集弹幕并保存到数据库中，用于分析弹幕，比如高能弹幕，发言人数，礼物数量，高频弹幕等

## 功能

1. B站上传优化，分P分开上传，支持上传重试、重试延迟、线路、并发、同时上传限制选项，暂时没有添加如手动重试等更多控制功能，如果有bug请反馈
2. 支持为不同分辨率提供不同渲染功能
3. webhook标题支持ejs模板引擎，具体使用见文档 ([#21](https://github.com/renmu123/biliLive-tools/issues/21))
4. 新增log直接查看功能
5. ffmpeg配置增加缩放算法，分辨率默认值修改为2880x1620
...
3. 支持为不同分辨率提供不同渲染功能
4. 手动取消任务时不会触发错误事件
5. 斗鱼弹幕元数据增加`video_start_time`参数
6. webhook标题支持预览输出
7. 移除完成后删除源文件和完成后打开文件夹功能，由于这两个选项在各个页面中或无效或有问题
8. 增加设置栏目
